...
- **Qwen models on AWS Bedrock** — The Bedrock LLM provider now routes `qwen.*` and `<region>.qwen.*` model IDs to an OpenAI-compatible chat completions body (the format AWS Bedrock exposes for Qwen serverless). Supported out of the box: `qwen.qwen3-next-80b-a3b`, `qwen.qwen3-coder-next`, `qwen.qwen3-vl-235b-a22b`, `qwen.qwen3-32b-v1:0`, `qwen.qwen3-coder-30b-a3b-v1:0` (AWS publishes some Qwen IDs with a `-v1:0` suffix and others without — verify via `aws bedrock list-foundation-models`). Default pricing and `MaxOutputTokens` entries are seeded for each model and can be overridden per project. System prompts are carried as the first message with `role="system"` (OpenAI style) rather than as a top-level `system` field, matching what Qwen on Bedrock expects. No schema duplication: the request/response format is shared with the OpenAI and Azure AI Foundry providers through a new `libs/go-common/llm/openaicompat` package (`BuildRequestBody`, `ParseResponseBody`, typed `APIError`); each provider keeps its own transport (HTTP POST vs Bedrock `InvokeModel`) and auth (Bearer token vs `api-key` header vs AWS SigV4). The OpenAI and Azure AI Foundry providers were refactored to use the shared package with no behavioural change; existing tests cover the new routing plus regression for the refactored providers.
