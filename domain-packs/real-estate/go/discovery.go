...
func getPromptsPath() string {
	if p := os.Getenv("DOMAIN_PACK_PATH"); p != "" {
		return filepath.Join(p, "real-estate", "prompts")
	}
	return "domain-packs/real-estate/prompts"
}

// getProfilesPath returns the path to profiles, checking env var on each call.
func getProfilesPath() string {
	if p := os.Getenv("DOMAIN_PACK_PATH"); p != "" {
		return filepath.Join(p, "real-estate", "profiles")
	}
	return "domain-packs/real-estate/profiles"
}

// Compile-time check: RealEstatePack implements DiscoveryPack.
var _ domainpack.DiscoveryPack = (*RealEstatePack)(nil)
...
// DomainCategories returns the real estate categories.
func (p *RealEstatePack) DomainCategories() []domainpack.DomainCategory {
	return []domainpack.DomainCategory{
		{
			ID:          "sales_navigator",
			Name:        "Sales Navigator",
			Description: "Real estate CRM with lead management, listing tracking, buyer-seller matching, property valuations, and agent performance analytics",
		},
	}
}
...
func (p *RealEstatePack) AnalysisAreas(categoryID string) []domainpack.AnalysisArea {
...
func (p *RealEstatePack) Prompts(categoryID string) domainpack.PromptTemplates {
...
func (p *RealEstatePack) ProfileSchema(categoryID string) map[string]interface{} {
