...
// Compile-time check: EcommercePack implements DiscoveryPack.
...
// DomainCategories returns the ecommerce store type categories.
func (p *EcommercePack) DomainCategories() []domainpack.DomainCategory {
	return []domainpack.DomainCategory{
		{
			ID:          "multi_category",
			Name:        "Multi-Category Store",
			Description: "Large online stores selling across multiple product categories such as electronics, appliances, and fashion (e.g., Amazon, eBay, Ozon)",
		},
	}
}
